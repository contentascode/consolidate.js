...
var fs = require('fs')
  ...
  , debug = require('debug')('consolidate.js')
...
exports.liquid.render = function(str, options, fn){
  return promisify(fn, function (fn) {
    ...
    try {
      ...
      options.ext = options.ext || '.liquid';

      context.onInclude(function (name, callback) {
        var extname = path.extname(name) ? '' : options.ext;
      ...
      /**
       * Add options.ext if there is not extension:
       */

      context._locals.layout = path.extname(context._locals.layout) ? '' : context._locals.layout + options.ext;
      // debug("Using template: " + context._locals.layout + " for file: " + context._locals.paths.href)
